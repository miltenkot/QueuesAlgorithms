...
@Suite("DoubleStackQueue Tests")
struct DoubleStackQueueTests {

    @Test("Queue is empty upon initialization")
    func testQueueIsEmptyInitially() {
        let queue = DoubleStackQueue<Int>()
    ...
    func testEnqueue() throws {
        let queue = DoubleStackQueue<String>()
    ...
    func testClear() throws {
        let queue = DoubleStackQueue<Character>()
        try queue.enqueue("X")
        try queue.enqueue("Y")
        #expect(queue.count == 2)
    ...
    func testLimitedCapacityEnqueueSuccess() throws {
        let queue = DoubleStackQueue<Double>(capacity: 3)
    ...
    func testLimitedCapacityEnqueueFailure() throws {
        let queue = DoubleStackQueue<String>(capacity: 2)
        try queue.enqueue("A")
        try queue.enqueue("B")
        #expect(queue.count == 2)
    ...
    func testLimitedCapacityClearAndReEnqueue() throws {
        let queue = DoubleStackQueue<Int>(capacity: 1)
        try queue.enqueue(1)
        #expect(queue.count == 1)
